...
class PrintLnParser(private val parserSelector: Map<TokenType, Parser>): Parser {
    override fun parse(tokens: List<Token>, currentIndex: Int): ParserResult {
        ...
        if (!areInitialTokensValid(tokens, currentIndex, openParenthesisIndex)) {
            return FailureResult("Expected println token followed by an open parenthesis at position $currentIndex\")", currentIndex)
        }
        val nextToken = next(tokens, openParenthesisIndex)
        val syntaxSubtree = getSyntaxSubtree(nextToken, tokens, currentIndex, parserSelector)
    ...
    private fun areInitialTokensValid(tokens: List<Token>, printLnIndex: Int, openParenthesisIndex: Int): Boolean {
        return isTokenValid(tokens, printLnIndex, TokenType.PRINTLN) && isTokenValid(tokens, openParenthesisIndex, TokenType.OPENPARENTHESIS)
    }

    private fun areFinalTokensValid(tokens: List<Token>, closeParenthesisIndex: Int): Boolean {
        return isTokenValid(tokens, closeParenthesisIndex, TokenType.CLOSEPARENTHESIS) && isEndOfStatement(tokens, nextIndex(closeParenthesisIndex))
    }

    private fun parsePrintLnSyntax(tokens: List<Token>, syntaxSubtree: ParserResult): ParserResult {
        return when (syntaxSubtree) {
            is SuccessResult -> {
                ...
                if (!areFinalTokensValid(tokens, closeParenthesisIndex)) {
                    return FailureResult("Expected a close parenthesis followed by a semicolon at position $closeParenthesisIndex", closeParenthesisIndex)
                }
    ...
    private fun builtParserResult(expressionNode: ExpressionNode, lastValidatedIndex: Int): ParserResult {
