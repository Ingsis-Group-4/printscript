...
class WhitespaceBeforeAndAfterOperatorRuleTest {
    ...
    fun testShouldReturnNodesUnchanged(){
        ...
        assertEquals(operatorNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getStart())
        assertEquals(operatorNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getEnd())
        assertEquals(firstExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getStart())
        assertEquals(firstExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getEnd())
        assertEquals(secondExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getStart())
        assertEquals(secondExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getEnd())
    }
    @Test
    fun testShouldChangeTheRightNodeOnly(){
        ...
        assertEquals(firstExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getStart())
        assertEquals(firstExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getEnd())
        assertEquals(newSecondExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getStart())
        assertEquals(newSecondExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getEnd())
    }
    @Test
    fun testShouldMoveEachNodeOneSpaceToTheLeft(){
        ...
        assertIs<ProgramNode>(result)
        assertIs<VariableDeclarationNode>(result.statements[0])
        assertEquals(newOperatorNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getStart())
        assertEquals(newOperatorNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getEnd())
        assertEquals(firstExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getStart())
        assertEquals(firstExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getEnd())
        assertEquals(newSecondExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getStart())
        assertEquals(newSecondExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getEnd())
    ...
    fun testWithComplexTerms(){
        ...
        val newSecondExpressionNode = SumNode(newThreeLiteralNode,newFourLiteralNode,newThirdOperatorNode,Position(1,26),Position(1,30))
        assertIs<ProgramNode>(result)
        assertIs<VariableDeclarationNode>(result.statements[0])
        assertEquals(newSecondOperatorNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getStart())
        assertEquals(newSecondOperatorNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getOperator().getEnd())
        assertEquals(newFirstExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getStart())
        assertEquals(newFirstExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getLeft().getEnd())
        assertEquals(newSecondExpressionNode.getStart(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getStart())
        assertEquals(newSecondExpressionNode.getEnd(),((result.statements[0] as VariableDeclarationNode).expression as SumNode).getRight().getEnd())
    }
    @Test
